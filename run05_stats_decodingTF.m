...
save([tfrpath, 'meandbpow.mat'], 'meandbpow');
...
pval = pnum / numruns;
%pnum2 = squeeze(sum(tmpda > mean(DA_all))) ./numruns
...
pthresh = 0.05 / (27*90); % bonferroni .05, on all bins
...
mxc = cell(length(HzRange), size(tmpp,1));
tmppPos = tmpp;
tmppPos(tmpt < 0) = 1; % only use right side t-tests

for nmr = 1:size(tmpp,1)
threshedP = squeeze(tmppPos(nmr, HzRange, 31:90)) < pthresh; %bonferoni thres
...
clustermat(clst).max_t       = [tvec(c(maxtind)), fvec(r(maxtind)), maxt];
clustermat(clst).max_da      = [tvec(c(maxdaind)), fvec(r(maxdaind)), maxda];
...
fprintf('\n max t  = %.3f at %.4fs and %i Hz', maxt, tvec(c(maxtind)), fvec(r(maxtind)));
fprintf('\n max DA = %.3f at %.4fs and %i Hz\n\n', maxda, tvec(c(maxdaind)), fvec(r(maxdaind)));
end
