...
% ==============
% use correctly pre-sorted (1:72) EEG-(DA)-RDMs
% paths and files
addpath('./func');
dapath      = './DAerp3/';
    ...
    RDM = squareform(RDMs{rdm})';
        for vp = 1:numel(da.subjects)
           for timebin = 1:size(da.data,4)
            ...
            tmpDA(isnan(tmpDA)) = 0; % make tmpDA square matrix
            tmpDA = tmpDA + tmpDA';  % make tmpDA square matrix
            DA = squareform(tmpDA)';
            rho(rdm, vp, timebin) = corr(RDM, DA, 'type', 'Spearman');
            model0 = fitlm(zscore(RDM), zscore(DA));
