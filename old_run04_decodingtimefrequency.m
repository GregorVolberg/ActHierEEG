...
fprintf([files.name{vp}, ' loaded\n']);
bsltimebins = [find(eeg.tfr.time == baseline(1)), find(eeg.tfr.time == baseline(2))];
% results matrix pre-allocation
DAmean = nan(nconditions, nconditions, size(eeg.tfr.powspctrm, 3), size(eeg.tfr.powspctrm, 4));
...
for condRun = 1:size(conds,1)
    ...
    DA  = zeros(size(eeg.tfr.powspctrm, 3), size(eeg.tfr.powspctrm, 4)); %pre-allocate
    %clear eeg
    
    for permrun = 1:numruns

    pseudoA = getPseudoTrials(selA, nbins, bsltimebins);
    pseudoB = getPseudoTrials(selB, nbins, bsltimebins);
    tmpDA  = zeros(size(DA)); %pre-allocate
